...
(function () {
	var width, height, largeHeader, canvas, ctx, points, target, animateHeader = true; (function () {
		...
		width = window.innerWidth; height = window.innerHeight; target = { x: width / 2, y: height / 2 }; largeHeader = document.getElementById("page-loader"); largeHeader.style.height =
			height + "px"; canvas = document.getElementById("demo-canvas"); canvas.width = width; canvas.height = height; ctx = canvas.getContext("2d"); points = []; for (var x = 0; x < width; x = x + width / 50)for (var y = 0; y < height; y = y + height / 50) { var px = x + Math.random() * width / 50; var py = y + Math.random() * height / 50; var p = { x: px, originX: px, y: py, originY: py }; points.push(p) } for (var i = 0; i < points.length; i++) {
